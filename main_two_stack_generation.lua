...
opt.iter_ph1 = torch.floor(opt.niter / 3)
opt.iter_ph2 = torch.floor(opt.niter / 3 * 2) 
...
local function get_G_model(opt)
	...
	--input_image1 :RGB color image 3 * 64 *64
	
	local input_image1 = nn.Identity()() 
	--imgz1: (8 * ndf) x 1 x 1
	local imgz1 = encoder(input_image1)
	--input_image2 :RGB color image 3 * 64 *64
	--imgz2: (8 * ndf) x 1 x 1
	local input_image2 = nn.Identity()()
	local imgz2 = encoder(input_image2)
	--allz size:516 x 1 x1
	local allz = nn.JoinTable(2)({imgz1, imgz2}) --the 1 dimension is batchsize
	--encoder input channel : nde = 8 * ndf + ncondition = 8 * 64 + 4 = 512 + 4 = 516
	local nde = ndf * 8 + ndf * 8
	...
	local Two_AE = nn.gModule({input_image1, input_image2}, {decoder_image})

	--graph.dot(Two_AE.fg, 'Two_AE forward', 'Two_AE forward')
	return Two_AE
end
...
local input_image1 = torch.Tensor(opt.batchSize, opt.nc, opt.fineSize, opt.fineSize)
local input_image2 = torch.Tensor(opt.batchSize, opt.nc, opt.fineSize, opt.fineSize)
...
if opt.gpu > 0 then
	...
	input_image = input_image:cuda(); input_image1 = input_image1:cuda(); input_image2 = input_image2:cuda();
	output_image = output_image:cuda();label = label:cuda()
...
local fDx = function(x)
	...
	--train with fake
	--input_image:copy(imagebatch)
	input_image1:copy(imagebatch)
	input_image2:copy(imagebatch)  --restruction 
	outputG = Gmodel:forward({input_image1, input_image2})

	--[[testing for the evaluation criterion:SSIM, PSNR
...
for epoch = 1, opt.niter do
	...
	for i = 1, math.min(dataset:size(), opt.ntrain), opt.batchSize do --opt.train is useless now
		...
		--debugger.enter()
		if epoch < opt.iter_ph1 then
			print("phase 1:")
			optim.adam(fDx, parametersD, D_optimconfig1)

			optim.adam(fGx, parametersG, G_optimconfig1)
		else if epoch >= opt.iter_ph1 and epoch < opt.iter_ph2 then
			print("phase 2:")
			optim.adam(fDx, parametersD, D_optimconfig2)

			optim.adam(fGx, parametersG, G_optimconfig2)		
			else
				print("phase 3:")
				optim.adam(fDx, parametersD, D_optimconfig3)

				optim.adam(fGx, parametersG, G_optimconfig3)		
			end	
		end			
